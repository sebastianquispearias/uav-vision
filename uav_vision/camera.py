...
class CamaraArduCam:
    ...
    picamera2 y ultralytics se importan la primera vez que se pide una
    foto, no al crear el objeto. Asi este archivo se puede importar en
    una laptop que no tiene ninguno de los dos instalados.
    """
    ...
    def ver_alvo(self, pos: Sequence[float], yaw: float) -> List[Deteccion]:
        ...
        for caja in resultados[0].boxes:
            if self._yolo.names[int(caja.cls[0])] != self.clase:
                continue
            x1, _y1, x2, y2 = caja.xyxy[0].cpu().numpy()
            detecciones.append({
                "px": float((x1 + x2) / 2),
