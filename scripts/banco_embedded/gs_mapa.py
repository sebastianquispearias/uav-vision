...
def _vector(poi):
    """The appearance vector of a POI, or None when the drone did not send one."""
    e = poi.get('emb')
    if not e or not FUSION_DISPONIBLE:
        return None
    try:
        v = _np.frombuffer(base64.b64decode(e), dtype=_np.float16).astype(_np.float32)
    except Exception:
        return None
...
def _mismo_objetivo(a, b):
    ...
    if ca and cb and ca != cb:
        return False
    radio = RADIOS.get(ca or cb, 3.5) if FUSION_DISPONIBLE else 3.5
    ...
    if va is None or vb is None:
        return True
    return float(_np.linalg.norm(va - vb)) <= EMB_DIST_MAX_MEDIDO
...
def fundir(por_dron):
    ...
    for dron, lista in por_dron.items():
        for poi in lista:
            for ya in salida:
                if str(dron) in ya['drones'] or not _mismo_objetivo(ya, poi):
                    continue
                ...
                # The metres moved, so the coordinates have to move with them. Leaving the
                # first drone's lat/lng would put the pin and its GPS reading in two places.
                ya['lat'], ya['lng'] = a_latlng(ya['x'], ya['y'], ESTADO['origen'])
                break
            else:
                nuevo = dict(poi)
